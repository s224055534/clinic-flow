...
export default function AnalyticsDashboard({ staff, role, selectedDate, refreshKey }) {
  const initialBounds = useMemo(() => monthBounds(selectedDate), [selectedDate]);
  ...
  useEffect(() => {
    ...
    if (staffId) parameters.set("staffId", staffId);
    const options = {
      headers: {
        "x-demo-role": role,
    ...
    async function loadAnalytics() {
      ...
      try {
        ...
        const responses = await Promise.all(
          paths.map((path) => fetch(`${path}?${parameters}`, options)),
        );
        const bodies = await Promise.all(responses.map((response) => response.json()));
        const failed = responses.findIndex((response) => !response.ok);
        if (failed >= 0) throw new Error(bodies[failed].error || "Could not load analytics.");
        ...
        if (active) {
          setAnalytics({
            ...
            statusDistribution: bodies[3].map((item) => ({ ...item, count: Number(item.count) })),
          });
        }
      } catch (loadError) {
      ...
      {analytics && !loading && (
        <>
          <section className="analytics-metrics">
            <MetricCard label="Appointments" value={analytics.summary.totalAppointments} />
            <MetricCard label="Booked hours" value={Number(analytics.summary.bookedHours).toFixed(1)} />
            <MetricCard label="Cancellation rate" value={`${analytics.summary.cancellationRate}%`} detail={`${analytics.summary.cancelledAppointments} cancellations`} />
            <MetricCard
              label="Busiest day"
              value={busiestDay?.date || "No appointments"}
              detail={busiestDay?.count ? `${busiestDay.count} appointments` : undefined}
            />
          </section>
          <section className="analytics-grid">
            ...
            <ChartCard title="Cancellation trend">
              <TrendLineChart data={analytics.trends.cancellations} color="#c85b5b" />
            </ChartCard>
            <ChartCard title="Clinician utilization">
              <ClinicianBarChart data={analytics.clinicians} />
              <div className="workload-list">
                {analytics.clinicians.map((item) => (
                  <p key={item.id}>{item.fullName}: {item.bookedHours} hours ({item.workloadPercentage}%)</p>
                ))}
              </div>
            </ChartCard>
